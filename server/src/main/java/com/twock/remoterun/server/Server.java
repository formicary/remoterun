...
public class Server implements NettyServer.ServerConnectionCallback {
  ...
  private void runCommand(String line) {
    ...
    if(connectedClients.isEmpty()) {
      ...
      ServerToClient.Builder builder = ServerToClient.newBuilder()
  ...
  private void closeInput(String line) {
    ...
    if(connectedClients.isEmpty()) {
      ...
      ClientConnection connection = connectedClients.iterator().next();
      ServerToClient.Builder builder = ServerToClient.newBuilder().setMessageType(CLOSE_STDIN).setRequestId(id);
      connection.getChannel().write(builder.build());
    }
  }
