package com.twock.remoterun.agent;
...
import com.twock.remoterun.agent.process.ProcessHelper;
import com.twock.remoterun.agent.process.ReadCallback;
  ...
  public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) throws Exception {
    ...
    if(RUN_COMMAND == message.getMessageType()) {
      ...
      try {
        ...
        write(RemoteRun.AgentToMaster.newBuilder().setMessageType(STARTED).setRequestId(requestId).build());
        processHelper.start();
      } catch(Exception e1) {
        log.info("Failed to start process " + runCommand, e1);
        // write a failure reply
        write(RemoteRun.AgentToMaster.newBuilder().setMessageType(EXITED)
